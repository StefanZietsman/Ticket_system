The project name

Service Ticket System


A clear, short and to-the-point description of your project.

The Service Ticket System makes it possible for customers to log an issue with heading and query including customer name, email and date. Logged service tickets can then be searched, updated and deleted once query has been resolved. This system will help document service queries. Includes unit test.


A table of contents

Installation of program.
Operating of program.
Unit test 'test_ticket_system.py'


Installation of program

Install all files and folders in one designated folder.


Operating of program

Open main.py and run code and you will be greeted by the main menu. Choose from the options listed to add, search, print and change and delete completed tickets. Each option will guide the user for specific actions. To exit program use exit option.


A section for credits

Thank you to the creators of Regular Expressions (RE) module which helps in validation of email addresses.


